...
def init():
	...
	for item in element.findall(GAMER_MAIN_XPATH):
		...
		Dir.AddToDir(	"Gamejournaal week " + week,		desc,		'flash://%s/src=%s&bx-jsactions=%s' % ("gamer.nl", quote(BASE_URL+str(item.find(".//a[@title]").attrib["href"])), quote('http://boxee.bartsidee.nl/js/gamer.js')), 	item.find(".//img").attrib["src"], 	properties)
