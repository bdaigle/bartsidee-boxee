...
class _Base:
    ...
    def __init__(self,path,protocol=pickle.HIGHEST_PROTOCOL):
        ...
        if self.exists():  
            try:
                raw  = mc.GetApp().GetLocalConfig().GetValue(self.path)
                data = zlib.decompress(binascii.unhexlify(raw))
                self.fields, next_id, records, indices = pickle.loads(data)
            except:
                print traceback.format_exc()

    def exists(self):
        try:
            return mc.GetApp().GetLocalConfig().GetValue(self.path)
        except:
            print traceback.format_exc()
            mc.GetApp().GetLocalConfig().Reset(self.path)
            return False
    ...
    def create(self,*fields,**kw):
        ...
        if self.exists():
            if mode is None:
                ...
                try:
                    mc.GetApp().GetLocalConfig().Reset(self.name)
                except:
                    print traceback.format_exc()
    ...
    def open(self):
        ...
        try:
            raw  = mc.GetApp().GetLocalConfig().GetValue(self.path)
            data = zlib.decompress(binascii.unhexlify(raw))
            self.fields, self.next_id, self.records, self.indices = pickle.loads(data)
        except:
            print traceback.format_exc()
            return self
    ...
    def commit(self):
        ...
        try:
            data   = pickle.dumps(tupple, self.protocol)
            raw = binascii.hexlify(zlib.compress(data))
            mc.GetApp().GetLocalConfig().SetValue(self.name, raw)
        except:
            print traceback.format_exc()
            mc.GetApp().GetLocalConfig().Reset(self.name)
